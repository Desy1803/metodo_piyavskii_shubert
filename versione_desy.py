...
def selection_function(func_name):
    ...
    global t
    if func_name == 'f1':
        f = lambda x: (1 / 6) * x ** 6 - (52 / 25) * x ** 5 + (39 / 80) * x ** 4 + (71 / 10) * x ** 3 - (
                    79 / 20) * x ** 2 - x + (1 / 10)
        t = (-1.5, 11, 13870)
    elif func_name == 'f2':
...
def selection_function_np(func_name):
    ...
    global tnp
    if func_name == 'f1':
        fnp = lambda x: (1 / 6) * x ** 6 - (52 / 25) * x ** 5 + (39 / 80) * x ** 4 + (71 / 10) * x ** 3 - (
                    79 / 20) * x ** 2 - x + (1 / 10)
        tnp = (-1.5, 11, 13870)
    elif func_name == 'f2':
...
def trova_min_Ri(x_intervals, L):
    ...
    for i in range(len(x_intervals)):
        ...
        if Ri < minimo1:
            minimo2 = minimo1
            min_inter2 = min_inter1
            minimo1 = Ri
            min_inter1 = x_intervals[i]
        elif Ri < minimo2 and min_inter1 != x_intervals[i]:
            minimo2 = Ri
            min_inter2 = x_intervals[i]
        
   
    min_values = [(min_inter1, minimo1)]
    if min_inter2 is not None:
        min_values.append((min_inter2, minimo2))
    numIterazioni+=len(x_intervals)
    return min_values
    ...
    while True:
        ...
        if interval1[1] - interval1[0] <= epsilon:
            if interval2 != None and interval2[1] - interval2[0] <= epsilon:
                ...
                numIterazioni+=k
                print("Numero di iterazioni: ", numIterazioni)
...
def main(func_name):
    ...
    # Creazione dell'array contenente le stringhe da "f1" a "f12"
    function_names = ["f1", "f2", "f3", "f4", "f5", "f6", "f7", "f8", "f9", "f10", "f11", "f12", "f13", "f14", "f15", "f16", "f17", "f18", "f19", "f20"]

    i=0
    epsilon = 0.00001
    #while i<20:
    #selection_function(function_names[i])
    #print("function: ", function_names[i])
    print("function: ", func_name)
    a = t[0]
    b = t[1]
    L = t[2]
    start_time = time.time()
    x_values, z_values = mps(a, b, L, epsilon)
    end_time = time.time()
